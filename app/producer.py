...
def get_interactions_from_redis(agent_id: str):
    """Retrieve interactions for a specific agent from Redis."""
...
def get_rabbitmq_connection():
    ...
    key = (Config.RABBITMQ_HOST, Config.RABBITMQ_PORT)
    if key not in _connection_pool or _connection_pool[key].is_closed:
        credentials = pika.PlainCredentials(Config.RABBITMQ_USER, Config.RABBITMQ_PASS)
        _connection_pool[key] = pika.BlockingConnection(
...
def get_output_file_from_agent(agent_id):
    client=MongoClient(Config.MONGODB_URI)
    db = client[Config.MONGODB_DB]  # Use your desired database name
    agent_doc = db["agents"].find_one({"_id": agent_id})
    if agent_doc and "output_file" in agent_doc:
        return agent_doc["output_file"]
    raise ValueError(f"No output_file found for agent_id: {agent_id}")
...
def process_records_in_batches_custom(records, max_tokens=1024, reserved_model_response_tokens=200, encoding_name="cl100k_base"):
    """
...
if __name__ == "__main__":
    ...
    try:
        data = get_interactions_from_redis(agent_id)
        if not data:
    ...
    batches = process_records_in_batches_by_session(
        records=data,
        max_tokens=1024,                  # set your desired max tokens
