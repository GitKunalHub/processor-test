    ...
    try:
        ...
        for topic_entry in response_data:
            ...
            if rb_entry and int_entry:
                ...
                # Safely parse interactions
                interactions_str = original_record.get("interactions", "[]")
                try:
                    parsed_interactions = ast.literal_eval(interactions_str)
                except (SyntaxError, ValueError):
                    parsed_interactions = []
                    logger.warning(f"Failed to parse interactions for interaction_id {interaction_id}, defaulting to empty list")
                combined_entry = {
                    **topic_entry,
                    **rb_entry,
                    **int_entry,
                    ...
                    "interactions": interactions_str,  # Keep original string if needed elsewhere
                    "dialog_turns": len(parsed_interactions),
        ...
        for session_id, entries in sessions.items():
            ...
            earliest_interactions_str = id_to_record[str(earliest_entry["interaction_id"])].get("interactions", "[]")
            latest_interactions_str = id_to_record[str(latest_entry["interaction_id"])].get("interactions", "[]")
            try:
                interactions_earliest = ast.literal_eval(earliest_interactions_str)
            except (SyntaxError, ValueError):
                interactions_earliest = []
            try:
                interactions_latest = ast.literal_eval(latest_interactions_str)
            except (SyntaxError, ValueError):
                interactions_latest = []
        ...
        db = mongo_client[Config.MONGODB_DB]  # Use your desired database name
